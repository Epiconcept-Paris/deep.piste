...
def prepare_sparkly(repo):
    ...
    sparkly_paths = os.path.join(
        package_dir, "data", "sparkly-maven-paths.url")
...
def sparkly_cp(source, dest):
    ...
    """
    java.java_job(main_class="fr.epiconcept.sparkly.Command", class_path=f"{os.path.join(get_home(), 'libs', 'jars')}", memory="1g", args=[
                  "storage", "cp", "from", source, "to", dest, "force", "true"])


def log(message):
    print(
        f"[\033[94m{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\033[0m] {message}")
...
def stat_df(df, disclose_limit=100, undisclosed_types={}, metadata=None):
    ...
    if metadata is not None:
        ...
        df = df[["column", "type", "empty", "unique",
                 "avg repetition", "description", "sensitivity", "values"]]
    return df
...
def cleandir(dirlist: Union[str, list], deldir=False) -> None:
    ...
    for directory in dirlist:
        for path in os.scandir(directory):
            ...
            cleandir(full_path, deldir=True) if os.path.isdir(
                full_path) else os.remove(full_path)
...
def sftp_get_available_size() -> float:
    ...
    try:
        ...
        raise ValueError(
            f'Expected type float is str (value=[{output}]). SFTP might be unreachable.')
