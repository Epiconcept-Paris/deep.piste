        ...
        if s:    # frame captured without any errors
            ...
            while (True):
                ...
                if len(codes) > 0:
                    ...
                    square = np.array([[shape[0].x, shape[0].y], [shape[1].x, shape[1].y], [
                                      shape[2].x, shape[2].y], [shape[3].x, shape[3].y]], np.int32)
                    b64key = codes[0].data.decode("ASCII")
                if len(square) > 0:
                    cv2.polylines(img, [square], True,
                                  (0, 255, 0), thickness=3)
                    text = "key found, please any key to continue"
                cv2.putText(img, text, (20, 20), font, 0.8,
                            (25, 25, 25), 2, cv2.LINE_AA)
                cv2.imshow("key extractor", img)
    except:
        cam.release()
        raise Exception(
            f"Error found while getting code from webcam {sys.exc_info()}")
...
def encrypt(infile, cryptfile, b64key):
    ...
    if (not isinstance(b64key, str)):
        raise ValueError(
            f"Key must be a a base64 encoded string, but got class {type(b64key)}")
    key = base64.decodebytes(b64key.encode("ASCII"))
    if (not isinstance(key, bytes) or len(key) != 32):
        raise ValueError(
            f"decoded Key must be a 32 byte (256bits) array, but got {type(key)} of length {len(key)}")
