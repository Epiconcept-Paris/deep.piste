...
def p06_001_get_dicom(server, port=11112, retrieveLevel='STUDY', limit=None, page_size=10, filter_field=None, filter_value=None):
    ...
    studies = apply_filter(
        dal.screening.depistage_pseudo(), filter_field, filter_value)
    ...
    for page in range(0, pages):
        chunk_uids = [uid for i, uid in enumerate(
            uids) if i % chunks == page - 1]
            ...
            if limit is not None:
                if nb_studies < limit:
                    if not os.path.exists(dest):
                        os.makedirs(dest)
                    get_dicom(key=uid, dest=dest, server=server, port=port,
                              title=title, retrieveLevel=retrieveLevel, silent=True)
                    nb_studies += 1
                else:
                    break
...
def apply_filter(df: pd.DataFrame, field: str = None, value: str = None) -> pd.DataFrame:
    ...
    if field is None and value is None:
        ...
        return df[(df[field] is True) & (pd.notna(df['DICOM_Studies']))]
    elif value == 'False':
        return df[(df[field] is False) & (pd.notna(df['DICOM_Studies']))]
    else:
...
def get_patient_random_id(df):
    ...
    for i in shorten_df.index:
        uids_i = list(json.loads(
            shorten_df['DICOM_Studies'][i]).values())[0][0]
...
def extract_positive_studies(server="127.0.0.1", port=11112, title="DCM4CHEE", retrieveLevel="STUDY"):
    ...
    df_without_nan_study_ids = keep_only_studies_with_images(
        df_with_positive_only)
    ...
    for study_id in df_without_nan_study_ids["DICOM_Study"]:
        ...
        get_dicom(key=study_id, dest=dest, server=server,
                  port=port, title=title, retrieveLevel=retrieveLevel, silent=True)
...
def filter_depistage_pseudo(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["DICOM_Study"] = df.apply(get_dicom_study_id, axis=1)
    return df


def get_dicom_study_id(row: pd.Series) -> pd.Series:
    """
...
def get_higher_acr_lvl(df: pd.DataFrame, index: int) -> int:
    ...
    """
    acr_lvls = [df['L1_ACR_SD'][index], df['L1_ACR_SG'][index],
                df['L2_ACR_SD'][index], df['L2_ACR_SG'][index]]
    normalized_acr_lvls = [int(acr_lvl) if not pd.isnull(
        acr_lvl) and acr_lvl.isnumeric() else 0 for acr_lvl in acr_lvls]
...
def calculate_l1_l2_result(df: pd.DataFrame) -> pd.DataFrame:
    df['L1_Result'] = df.apply(get_l1_result, axis=1)
    df['L2_Result'] = df.apply(get_l2_result, axis=1)
    return df
...
if __name__ == '__main__':
    extract_positive_studies(
        server="127.0.0.1", port=11112, title="DCM4CHEE", retrieveLevel="STUDY")
